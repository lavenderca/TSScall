...
import os
import argparse
from TSScall import sortList, isLessThan, isWithin, readInReferenceAnnotation
    ...
    with open(bed_fn) as f:
        for line in f:
            if not ('track' in line or 'browser' in line or line == '\n'):
                chromosome, start, end, name, score, strand = line.strip().split()[:6]
                bed_list.append({
...
def compareTo(fn_1, fn_2, list_1, list_2):
    sys.stdout.write('Bed file ' + fn_1 + ' contains ' + str(len(list_1)) + ' entries.\n')
...
compareTo(args.bed_2, args.bed_2, bed_list_2, bed_list_1)
